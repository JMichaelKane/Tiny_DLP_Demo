...
func (d *Docx) ReplaceCustom(oldString string, newString string, num int) (err error) {
	oldString, err = encode(oldString)
	if err != nil {
		return err
	}
	...
	d.customs = strings.Replace(d.customs, oldString, newString, num)
...
func ReadDocx(reader ZipData) (*ReplaceDocx, error) {
	...
	customs, err := readCustom(reader.files())
	if err != nil {
		return nil, err
	}

	headers, footers, _ := readHeaderFooter(reader.files())
	return &ReplaceDocx{zipReader: reader, content: content, links: links,   customs: customs,headers: headers, footers: footers}, nil
}
...
func readCustom(files []*zip.File) (text string, err error) {
	var documentFile *zip.File
	documentFile, err = retrieveCustomDoc(files)
	if err != nil {
		return text, err
	}
	var documentReader io.ReadCloser
...
func (d *Docx)readCustom2(files []*zip.File) (text string, err error) {
	var documentFile *zip.File
	documentFile, err = retrieveCustomDoc(files)
	if err != nil {
		return text, err
...
func retrieveCustomDoc(files []*zip.File) (file *zip.File, err error){
	for _, f := range files {
